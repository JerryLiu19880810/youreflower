					...
					<div class="level-1">
						...
						<div class="nav nav-tabs flex-lg-column mt-md-3 lavalamp">
							<?php foreach ($product_cats as $key => $product_cat):
								$active = $key == 0 ? 'active' : '';
								?>
								<a class="nav-item nav-link <?= $active ?>" data-filter="<?= $key + 1 ?>">
									<?= $product_cat->name ?>
								</a>
							<?php endforeach; ?>
						</div>
				...
				<div class="row gutter-2 filtr-container imagesloaded">
					...
					<?php foreach ($product_cats as $key => $product_cat):
						...
						<div class="col-12 filtr-item" data-category="<?= $key + 1 ?>" data-sort="value">
							<div class="owl-carousel owl-carousel--mask visible" data-items="[2,2,2,1]"
								data-loop="true" data-margin="10" data-nav="true">
								<?php
		...
		<div class="row gutter-1">
			...
			foreach ($collect_products as $collect_product_id):
				...
				if ($collect_product_id):
					...
					$product = wc_get_product($collect_product_id) ?? null;
					if ($product):
						$attachment_ids = $product->get_gallery_image_ids();
						...
						<div class="col-6 col-lg-3">
							<a href="<?= $product_url; ?>">
								<figure class="category category--alt">
									<div class="equal"><span class="image"
											style="background-image: url(<?= $product_image; ?>)"></span></div>
									<figcaption>
										<?= $product_title; ?>
									</figcaption>
								</figure>
